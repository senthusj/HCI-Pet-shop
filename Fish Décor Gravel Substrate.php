...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:Fish Décor Gravel Substrate.php");
			}
			else
			{
				echo "<script>alert('item already added')</script>";
				echo "<script>window.location='Fish Décor Gravel Substrate.php'</script>";
			}
	}
	else
	{
...
<div class="anurow">
<?php
$query="SELECT * FROM items WHERE category_id=16";
...
if(mysqli_num_rows($result)>0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		<form method="POST" action="Fish Décor Gravel Substrate.php?action=add&id=<?php echo $row['item_id']; ?>" >
		...
		<div class="addtocart">
		<a href="Fish Décor Gravel Substrate.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="add-to-cart" value="Add to cart" /></a>
		</div>
		</form>
		</div>
		</div>
