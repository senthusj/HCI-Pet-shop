...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:FISH.html.php");
			}
			else
			{
				echo "<script>alert('item already added')</script>";
				echo "<script>window.location='FISH.html.php'</script>";
			}
	}
	else
	{
...
if(mysqli_num_rows($result)>0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		?>
		 <div class="anucolumn">
		<div class="anucontent" style="width:225px; height:500px;">
		
		<form method="POST" action="FISH.html.php?action=add&id=<?php echo $row['item_id']; ?>" >
		...
		<div class="addtocart">
		<a href="FISH.html.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="add-to-cart" value="Add to cart" /></a>
		</div>
		</form>
		</div>
		</div>
