...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:search_item.php");
			...
			else
			{
				...
				echo "<script>window.location='search_item.php'</script>";
			}
	}
	else
	{
...
.tt-suggestion p {
	...
	<body>
		...
		<div class="row">
				...
				<div class="headercolumn" style="width:23%">
				 ...
				 <input type="text" name="typeahead" class="typeahead" style="width:253px;" placeholder="SEARCH....">
					<button type="submit" class="btn btn-primary btn-lg" style="float:right;" id="mybutton" name="btnsearch"><i class="fa fa-search"></i></button>
					<?php
					if(isset($_POST['typeahead']))
					{
						$_SESSION['iname']=$_POST['typeahead'];
				...
				<div class="styles2">
					...
					<button onclick="myFunctions()" class="dropbtns">Select Category</button>
					  <div id="myDropdowns" class="dropdowns-content" onMouseOut="funshowajihide()">
						...
						<a href="FISH.php.php">Fish</a>
						<a href="Fish food.php">Fish Food</a>
						<a href="Fish Tank.php">Fish Tank</a>
						<a href="Fish Decor Gravel Substrate">Fish Things</a>
	...
	if(isset($_SESSION['iname']))
	{
		...
		LIKE '%$iname%' OR item_link LIKE '$iname%' OR item_link LIKE '%$iname%'";
		...
		<h1><?php echo $result_count;?>Results Found</h1>
		...
		$search_ress="SELECT * FROM items WHERE item_name 
		LIKE '%$iname%' OR item_link LIKE '$iname%' OR item_link LIKE '%$iname%'";
		$result_gets=$con->query($search_ress);
		
		
		
		
		
		if($result_gets->num_rows>0)
		{
			
			
			while($row=$result_gets->fetch_array())
			{
				$return_arr[]=$row['item_name'];
				$ilink=$row['item_link'];
				?>
		...
		<form method="POST" action="search_item.php?action=add&id=<?php echo $row['item_id']; ?>" >
		...
		<div class="addtocart">
		<a href="search_item.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="add-to-cart" value="Add to cart" /></a>
		</div>
		</form>
	...
	{
		echo"<div class='container'><h2 style='color:green; font-size:23px;'>Search Something.....!</h2></div>";
	}
