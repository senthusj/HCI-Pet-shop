...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:reptiles.php");
			...
			else
			{
				...
				echo "<script>window.location='reptiles.php'</script>";
			}
	}
	else
	{
...
<div class="anurow">
<?php
$query="SELECT * FROM items WHERE category_id=6";
...
if(mysqli_num_rows($result)>0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		<form method="POST" action="reptiles.php?action=add&id=<?php echo $row['item_id']; ?>" >
		...
		<div class="addtocart">
		<a href="reptiles.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="add-to-cart" value="Add to cart" /></a>
		</div>
		</form>
		</div>
		</div>
