...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:small pet.php");
			}
			else
			{
				echo "<script>alert('item already added')</script>";
				echo "<script>window.location='small pet.php'</script>";
			}
	}
	else
	{
...
<div class="anurow">
<?php
$query="SELECT * FROM items WHERE category_id= 10;";
...
if(mysqli_num_rows($result)>0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		<form method="POST" action="small pet.php?action=add&id=<?php echo $row['item_id']; ?>" >
		...
		<div class="addtocart">
		<a href="small pet.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="add-to-cart" value="Add to cart" /></a>
		</div>
		</form>
		</div>
		</div>
