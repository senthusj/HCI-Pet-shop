...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:Fish food.php");
			...
			else
			{
				...
				echo "<script>window.location='Fish food.php'</script>";
			}
	}
	else
	{
...
<div class="anurow">
<?php
$query="SELECT * FROM items WHERE category_id=14";
...
if(mysqli_num_rows($result)>0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		<div class="addtocart">
		<a href="Fish food.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="add-to-cart" value="Add to cart" /></a>
		</div>
		</form>
		</div>
		</div>
