...
$sqlget="SELECT max(add_id) FROM addcart";
		...
		header("location:Fish.php");
			...
			else
			{
				...
				echo "<script>window.location='Fish.php'</script>";
			}
	}
	else
	{
...
<html lang="en">
	...
	<body>
		...
		<div class="row">
				...
				<img src="images\as.png" style="width:190px; height:130px; float:left;">
				</div>
				...
				if(!$showdetail){
					...
					?>	
						<a href="order.php" style=" width:50px; height:45px; margin-top:13px;float:right; margin-right:-240px;"><i style="font-size:40px" class="fa">&#xf07a;</i></a>
...
<div class="anurow">
<?php
$query="SELECT * FROM items WHERE category_id=12;";
...
if(mysqli_num_rows($result)>0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		<form method="POST" action="Fish.php?action=add&id=<?php echo $row['item_id']; ?>" >
		...
		<a href="bird.php?mes=<?php echo $row['item_id'];  ?>"> <input type="submit" name="add_to_cart" style="margin-top:5px;" class="btn btn-success" value="Add to cart" /></a>
		</form>
		</div>
		</div>
	<?php	
...
?>
	...
	if(isset($_GET['mes']))
	{
		...
		$sqlg="SELECT * FROM items WHERE item_id='$a'";
		...
		if($results->num_rows>0)
		{
			while($row = $results->fetch_assoc()) 
			{
				...
				$sqlsend="INSERT INTO orders(customer_id,Price,item_name,order_image) values('$myid','$order_price','$item_name','$image_upload')";
				$resultf=$con->query($sqlsend);
			}
